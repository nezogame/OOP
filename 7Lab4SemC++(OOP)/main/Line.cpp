...
Line::Line() {
	...
	line = new RectangleShape(Vector2f(150, 5));
	color = Color(160, 87, 69);
	new_color = color;
...
RectangleShape* Line::get_figure() {
	return line;
}
Color Line::get_color() {
	return color;
}
Color Line::get_new_color() {
	return new_color;
}
Color Line::get_transparent_color() {
...
void Line::change_color(Color color) {
	new_color = color;
	line->setFillColor(new_color);
}
...
void Line::auto_move(Vector2f mouse) {
